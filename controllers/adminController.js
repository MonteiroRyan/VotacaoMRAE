...
const adminController = {
  ...
  async criarUsuario(req, res) {
    ...
    try {
      const { cpf, nome, tipo, senha, municipio_id, ativo } = req.body;
      ...
      if (!tiposValidos.includes(tipo)) {
        return res.status(400).json({
          success: false,
          message:
            "Tipo de usuário inválido.  Use: ADMIN, PREFEITO, REPRESENTANTE, GOVERNADOR ou SECRETARIO",
        });
      }

      // GOVERNADOR e SECRETARIO não precisam de município
      if (["PREFEITO", "REPRESENTANTE"].includes(tipo) && !municipio_id) {
        return res.status(400).json({
          success: false,
          message: "Município é obrigatório para PREFEITO e REPRESENTANTE",
        });
      }

      // ADMIN precisa de senha
      if (tipo === "ADMIN" && !senha) {
        return res.status(400).json({
          success: false,
          message: "Senha é obrigatória para ADMIN",
        });
      }
      ...
      await pool.query(
        `INSERT INTO usuarios (cpf, nome, senha, tipo, municipio_id, ativo) 
         VALUES (?, ?, ?, ?, ?, ?)`,
        [cpf, nome, senhaHash, tipo, municipio_id || null, ativo ? 1 : 0]
      );
  ...
  async atualizarUsuario(req, res) {
    ...
    try {
      ...
      if (
        senha &&
        (tipo === "ADMIN" || usuariosExistentes[0].tipo === "ADMIN")
      ) {
